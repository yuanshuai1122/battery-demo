...
def handle_files(file_with_path: str):
    ...
    # 只保留需要的数据
    df = df[['步骤时间', '电流/A', '容量/Ah', '电压/V', '辅助温度/℃', '工步状态']]
    # 保留符合条件的行
    df = df[(df['工步状态'] == 'CCC') | (df['工步状态'] == 'CVC')]

    # 处理新文件路径
    file_split = file_with_path.split('/')
    new_file_name = 'new_' + file_split[-1]
...
def train_model(file_with_path):
    ...
    # 转换步骤时间列
    features['步骤时间'] = features['步骤时间'].apply(str_to_seconds)
